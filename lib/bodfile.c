...
bodfileopen(const char *filename)
{
    ...
    data_out.bed_offset_byte = 0;

    data_out.bod_buf_len = 0;
    data_out.bod_data_buf = NULL;
...
bodfileclose(BODFILE_ptr bod_data)
{
    ...
    if (bod_data->bod_data_buf) {
        free(bod_data->bod_data_buf);
        bod_data->bod_data_buf = NULL;
    }
...
opireadlines(BODFILE_ptr bod_data, OPI_LINE_ptr opi_lines, uint32_t line_num)
{
    ...
    uint32_t line_counter = 0;
    while (opireadline(bod_data, &(opi_line[line_counter])) == 0) {
        line_counter++;
    }
...
bodreaddata(BODFILE_ptr bod_data, double *bod_readout, uint32_t readout_len)
{
    ...
    if (fread(bod_readout, readout_len, fin) != readout_len) {
