...
{
    const char *bed_name = argv[1];
    uint32_t indi_num = 360;
    uint32_t vari_num = 9596452;
    BED_DATA_ptr bed_data = init_bed_data_struct(bed_name, 360, 9596452);
    for (int i = 0; i < 100; i++) {
        read_bed_by_variant(bed_data, NULL);
        for (int j = 0; j < indi_num; j++) {
            printf("%d ", (bed_data->data)[j]);
        }
        printf("\n");
    }
